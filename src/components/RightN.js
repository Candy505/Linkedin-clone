  ...
  useEffect(() => {
    const fetchUsernames = async () => {
      try {
        ...
        const fetchedUsernames = querySnapshot.docs.map(doc => doc.data().name);
        const fetchedstatus = querySnapshot.docs.map(doc => doc.data().status);
    ...
    <div className="mr-40 md:mr-1 md:-mt-10">
      ...
      <div className="text-center overflow-hidden mb-8 mx-2 bg-slate-50 rounded relative shadow-l border-solid  border border-gray-300 ">
        ...
        { usernames.map((username, index) => (
          ...
          <div  key = {index} className="text-center overflow-hidden mb-8 mx-2 bg-slate-50 rounded relative shadow-xl border-solid  border border-gray-300 w-60 h-80">
            <div className="py-12 ">
